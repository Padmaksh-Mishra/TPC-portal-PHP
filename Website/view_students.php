    ...
    <?php
      if (count($eligible_companies) > 0) {
          ...
          foreach ($eligible_companies as $company){
            echo '<tr><td>' . $company["company"] . '</td><td>' . $company["role"] . '</td><td>' . $company["package"] . '</td><td>' . $company["interview_mode"] . '</td><td>' . $company["req_since"] . '</td><td><button><a href="apply.php?company_id=' . $company["company_id"] . '&role=' . $company["role"] . '">Apply</button></button></td></tr>';
        }
        echo "</tbody></table>";
      } else {
